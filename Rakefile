...
task :new_post, :title do |t, args|
  ...
  open(filename, 'w') do |post|
    ...
    post.puts "tags: [#{tags}]"
    post.puts "image:"
    page.puts "  feature: #{randomTexture()}"
    post.puts "  credit: "
...
task :deploy do
  ...
  # sh "rm -rf _deploy/*"
  sh "cp -r _site/* ../mono6823.github.com-master/"
  puts "! Change directory master"
  cd "../mono6823.github.com-master" do
...
def randomTexture()
  l = Array.new
  Dir.chdir("images")
  Dir.glob("texture_*.jpg") do |f|
    l.push(File.basename(f))
  end
  Dir.chdir("..")
  l.shuffle.sample
end
